...
# -*- coding: utf-8 -*-
"""
Small, isolated text-based building extraction used as a fallback by
filename_building_parser.get_building_with_confidence().


This replaces in-file duplicates (e.g. an older `normalise_building_name`
inside local_batch_ingest.py). Keep it simple and conservative.
"""
    ...
    for n in range(5, 1, -1):
        for i in range(0, max(0, len(words) - n + 1)):
            ...
            matches = get_close_matches(
                candidate, known_buildings, n=1, cutoff=TEXT_CUTOFF)
            if matches:
                return matches[0]
